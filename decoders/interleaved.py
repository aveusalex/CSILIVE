...
def __find_bandwidth(incl_len):  ## trocar o incl_len pelos dados todos ou tamanho deles ja
    '''
...
def read_pcap(pcap_filepath, bandwidth=0, nsamples_max=0):
    ...
    pcap_filesize = os.stat(pcap_filepath).st_size  ## substituir aqui pelo tamanho do packet
    with open(pcap_filepath, 'rb') as pcapfile:
        fc = pcapfile.read()
    
    if bandwidth == 0:  ##especificar a largura de antemão, ou tamanho do packet
    ...
    ptr = 24  # nao temos global, packet, nem network info

    nsamples = 0
    while ptr < pcap_filesize:  ## talvez nao precisamos desse while por termos apenas 1 frame
        # Read frame header  ## esta lendo o packet header
...
############################## essa é a parte que nos interessa ##################################
        ptr += 50  ## nos temos informacoes a partir daqui
